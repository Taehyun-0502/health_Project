...
public class CheckInoutService {
    ...
    // PT 출석 접수 (키오스크)
    // 1. 계정 검증 -> 2. 유효한 PT 계약(4) + 잔여횟수 + 담당 트레이너 확인 -> 3. 하루 1회 제한
    // -> 4. 미확인 상태로 출석 기록 (잔여횟수 차감은 트레이너 확인 시점에 수행)
    public CheckInoutDTO ptCheckIn(MemberDTO credential) throws Exception {
        MemberDTO member = verifyMember(credential);

        ContractDTO contract = checkInoutMapper.findActiveContract(member.getUsername(), 4L);
        if (contract == null) {
            throw new IllegalStateException("이용 가능한 PT 계약이 없습니다.");
        }
        int remaining = remainingCount(contract);
        if (remaining <= 0) {
            throw new IllegalStateException("잔여 PT 횟수가 없습니다.");
        }
        ...
        row.setMemberName(member.getName());
        row.setRemainingCount(remaining); // 차감 전 잔여횟수 (확인 후 -1)
        return row;
    }
    ...
    public int confirmPt(Long inoutId, Long trainerUsername) throws Exception {
        ...
        ContractDTO contract = checkInoutMapper.findActiveContract(row.getUsername(), 4L);
        if (contract == null) {
            throw new IllegalStateException("회원의 유효한 PT 계약이 없습니다.");
        }
    ...
    // PT 잔여횟수 계산 = 계약 총 횟수(quantity, 스냅샷) - h_pt_manage 사용횟수
    private int remainingCount(ContractDTO contract) throws Exception {
        int total = contract.getQuantity() == null ? 0 : contract.getQuantity();
        return total - checkInoutMapper.findUsedCount(contract.getDataId());
    }
}
